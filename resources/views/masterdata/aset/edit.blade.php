    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-xl font-semibold text-gray-900">Edit Aset</h1>
                ...
                <div class="flex space-x-2">
                    ...
                    <button type="submit" form="assetForm" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700">Simpan Perubahan</button>
                </div>
            </div>
            ...
            <form id="assetForm" action="{{ route('aset.update', $asset->id_assets) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nama_asset" id="nama_asset" required
                        ...
                        <div>
                            ...
                            <select name="tipe_asset" id="tipe_asset" required onchange="handleTipeChange(this.value)"
                                ...
                                <option value="{{ $val }}" {{ old('tipe_asset', $asset->tipe_asset) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="text" name="harga_perolehan" id="harga_perolehan" required
                                       value="{{ number_format((float) preg_replace('/[^0-9]/', '', old('harga_perolehan', $asset->harga_perolehan)), 0, ',', '.') }}"
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span class="text-gray-500 sm:text-sm">Rp</span></div>
                                <input type="text" name="nilai_sisa" id="nilai_sisa" required
                                       value="{{ number_format((float) preg_replace('/[^0-9]/', '', old('nilai_sisa', $asset->nilai_sisa)), 0, ',', '.') }}"
                                       class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                                       oninput="formatCurrency(this)">
                            </div>
                        </div>
                        <div>
                            ...
                            <input type="number" name="masa_manfaat" id="masa_manfaat" min="1" required
                                   value="{{ old('masa_manfaat', $asset->masa_manfaat) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                        <div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <select id="depreciation_method_select"
                            ...
                            <option value="sum_of_years_digits" {{ old('depreciation_method', $asset->depreciation_method)==='sum_of_years_digits'?'selected':'' }}>Jumlah Angka Tahun</option>
                            <option value="double_declining" {{ old('depreciation_method', $asset->depreciation_method)==='double_declining'?'selected':'' }}>Saldo Menurun Ganda</option>
                        </select>
                        <input type="hidden" name="depreciation_method" id="depreciation_method"
                               value="{{ old('depreciation_method', $asset->depreciation_method) }}">
...
document.addEventListener('DOMContentLoaded', function () {
    handleTipeChange(document.getElementById('tipe_asset').value);
});
</script>
@endpush
@endsection
