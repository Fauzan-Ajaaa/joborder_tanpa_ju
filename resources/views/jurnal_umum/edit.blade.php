...
@push('scripts')
<script>
let itemIndex = {{ $journal->items->count() }};
...
// Initialize dengan existing items
...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-gray-900">Edit Jurnal Umum</h1>
    ...
    <form action="{{ route('jurnal-umum.update', $journal) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow sm:rounded-lg p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="transaction_date" id="transaction_date" value="{{ old('transaction_date', $journal->transaction_date->format('Y-m-d')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description', $journal->description) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white shadow sm:rounded-lg p-6">
            ...
            <div id="items-container">
                @foreach($journal->items as $index => $item)
                    <div class="item-row border rounded-lg p-4 mb-4 bg-gray-50">
                        <div class="flex justify-between items-start mb-3">
                            <h4 class="text-sm font-medium text-gray-700">Transaksi #{{ $index + 1 }}</h4>
                            <button type="button" onclick="removeItem(this)" class="text-red-600 hover:text-red-800 text-sm">Hapus</button>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700">Akun</label>
                                <select name="items[{{ $index }}][chart_of_account_id]" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                    <option value="">-- Pilih Akun --</option>
                                    @foreach($accounts as $account)
                                        <option value="{{ $account->id }}" data-code="{{ $account->account_code }}" @selected($item->chart_of_account_id == $account->id)>
                                            {{ $account->account_code }} - {{ $account->account_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Debit</label>
                                <input type="number" step="0.01" min="0" name="items[{{ $index }}][debit]" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" value="{{ $item->debit }}" onchange="calculateTotals()">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Kredit</label>
                                <input type="number" step="0.01" min="0" name="items[{{ $index }}][credit]" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" value="{{ $item->credit }}" onchange="calculateTotals()">
                            </div>
                        </div>
                        <div class="mt-3">
                            <label class="block text-sm font-medium text-gray-700">Keterangan</label>
                            <input type="text" name="items[{{ $index }}][description]" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" value="{{ $item->description }}" placeholder="Keterangan transaksi (opsional)">
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="mt-6 border-t pt-4">
                <div class="grid grid-cols-3 gap-4 text-lg font-bold">
                    ...
                    <div class="text-right">
                        ...
                        <div id="total-debit" class="text-emerald-600">Rp {{ number_format($journal->total_debit, 0, ',', '.') }}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm text-gray-600">Total Kredit:</div>
                        <div id="total-credit" class="text-rose-600">Rp {{ number_format($journal->total_credit, 0, ',', '.') }}</div>
                    </div>
                </div>
