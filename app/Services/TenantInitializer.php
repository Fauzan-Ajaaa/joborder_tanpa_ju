...
{
    public static function initialize(Company $company)
    {
        DB::transaction(function () use ($company) {
            self::seedUnits($company->id);
            self::seedCOA($company->id);
        });
    }

    private static function seedUnits($companyId)
    {
        $units = [
            ['name' => 'EKOR', 'code' => 'EKOR'],
        ...
        foreach ($units as $unit) {
            Unit::firstOrCreate(
                ['code' => $unit['code'], 'company_id' => $companyId],
                [
                    'name' => $unit['name'],
                    'is_active' => true,
                ]
    ...
    private static function seedCOA($companyId)
    {
        $accounts = [
            // ASSET
        ...
        foreach ($accounts as $account) {
            ChartOfAccount::firstOrCreate(
                ['code' => $account['code'], 'company_id' => $companyId],
                [
                    ...
                    'opening_balance'         => 0,
                    'normal_balance_position' => self::getNormalBalance($account['type']),
                    'is_active'               => true,
    ...
    private static function getNormalBalance(string $type): string
    {
        return match($type) {
            'asset', 'expense' => 'debit',
            default            => 'credit',
